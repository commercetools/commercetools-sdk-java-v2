...
public class CategoryFixtures {
    ...
    public static Category createCategory() {
        ...
        TypeDraft typeDraft = TypeDraftBuilder.of().key(key).name(
            CommercetoolsTestUtils.randomLocalizedString()).resourceTypeIds(
                Arrays.asList(ResourceTypeId.CATEGORY)).fieldDefinitions(
                    Arrays.asList(FieldDefinitionBuilder.of().type(CustomFieldStringType.of()).name(value).label(
                        localizedString).required(false).inputHint(TypeTextInputHint.SINGLE_LINE).build())).build();

        Type type = CommercetoolsTestUtils.getProjectRoot().types().post(typeDraft).executeBlocking().getBody();

        CategoryDraft categoryDraft = CategoryDraftBuilder.of().key(CommercetoolsTestUtils.randomKey()).name(
            localizedString).description(localizedString).slug(localizedString).orderHint(
                CommercetoolsTestUtils.randomString()).externalId(CommercetoolsTestUtils.randomId()).metaTitle(
                    localizedString).metaDescription(localizedString).metaKeywords(localizedString).custom(
                        CustomFieldsDraftBuilder.of().type(
                            TypeResourceIdentifierBuilder.of().id(type.getId()).build()).build()).assets(
                                Arrays.asList(AssetDraftBuilder.of().key(CommercetoolsTestUtils.getProjectKey()).name(
                                    localizedString).sources(Arrays.asList(assetSource)).build())).build();
    ...
    public static Category deleteCategory(final String id, final Long version) {
        ...
        return CommercetoolsTestUtils.getProjectRoot().categories().withId(id).delete().withVersion(
