...
public class ChannelIntegrationTests {
    ...
    public void getById() {
        ChannelFixtures.withChannel(channel -> {
            Channel queriedChannel = CommercetoolsTestUtils.getProjectRoot().channels().withId(
                channel.getId()).get().executeBlocking().getBody();
    ...
    public void query() {
        ChannelFixtures.withChannel(channel -> {
            ChannelPagedQueryResponse response = CommercetoolsTestUtils.getProjectRoot().channels().get().withWhere(
                "id=" + "\"" + channel.getId() + "\"").executeBlocking().getBody();
    ...
    public void update() {
        ChannelFixtures.withUpdateableChannel(channel -> {
            ...
            Channel updateChannel = CommercetoolsTestUtils.getProjectRoot().channels().withId(channel.getId()).post(
                ChannelUpdateBuilder.of().actions(updateActions).version(
                    channel.getVersion()).build()).executeBlocking().getBody();
