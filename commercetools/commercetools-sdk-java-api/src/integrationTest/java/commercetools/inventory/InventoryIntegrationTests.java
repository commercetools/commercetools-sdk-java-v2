...
public class InventoryIntegrationTests {
    ...
    public void createAndDelete() {
        InventoryEntryDraft inventoryEntryDraft = InventoryEntryDraftBuilder.of().sku(
            CommercetoolsTestUtils.randomString()).quantityOnStock(10L).build();
        ...
        InventoryEntry deletedInventoryEntry = CommercetoolsTestUtils.getProjectRoot().inventory().withId(
            inventoryEntry.getId()).delete().executeBlocking().getBody();
    ...
    public void getById() {
        InventoryEntryFixtures.withInventoryEntry(inventoryEntry -> {
            InventoryEntry queriedInventoryEntry = CommercetoolsTestUtils.getProjectRoot().inventory().withId(
                inventoryEntry.getId()).get().executeBlocking().getBody();
    ...
    public void query() {
        InventoryEntryFixtures.withInventoryEntry(inventoryEntry -> {
            InventoryPagedQueryResponse inventoryPagedQueryResponse = CommercetoolsTestUtils.getProjectRoot().inventory().get().withWhere(
                "id=" + "\"" + inventoryEntry.getId() + "\"").executeBlocking().getBody();
    ...
    public void update() {
        InventoryEntryFixtures.withUpdatableInventoryEntry(inventoryEntry -> {
            ...
            InventoryEntry updatedInventoryEntry = CommercetoolsTestUtils.getProjectRoot().inventory().withId(
                inventoryEntry.getId()).post(
                    InventoryEntryUpdateBuilder.of().actions(updateActions).version(
                        inventoryEntry.getVersion()).build()).executeBlocking().getBody();
