...
public class ProductDiscountIntegrationTests {
    ...
    public void createAndDelete() {
            ...
            CommercetoolsTestUtils.randomLocalizedString()).key(CommercetoolsTestUtils.randomKey()).description(
                CommercetoolsTestUtils.randomLocalizedString()).value(
                    ProductDiscountValueRelativeDraftBuilder.of().permyriad(1000L).build()).predicate(
                        "product.key=\"random-key\"").sortOrder("0.4").isActive(false).validFrom(
        ...
        ProductDiscount deletedProductDiscount = CommercetoolsTestUtils.getProjectRoot().productDiscounts().withId(
            productDiscount.getId()).delete().withVersion(productDiscount.getVersion()).executeBlocking().getBody();
    ...
    public void getById() {
        ProductDiscountFixtures.withProductDiscount(productDiscount -> {
            ProductDiscount queriedProductDiscount = CommercetoolsTestUtils.getProjectRoot().productDiscounts().withId(
                productDiscount.getId()).get().executeBlocking().getBody();
    ...
    public void getByKey() {
        ProductDiscountFixtures.withProductDiscount(productDiscount -> {
            ProductDiscount queriedProductDiscount = CommercetoolsTestUtils.getProjectRoot().productDiscounts().withKey(
                productDiscount.getKey()).get().executeBlocking().getBody();
    ...
    public void query() {
        ProductDiscountFixtures.withProductDiscount(productDiscount -> {
            ProductDiscountPagedQueryResponse response = CommercetoolsTestUtils.getProjectRoot().productDiscounts().get().withWhere(
                "id=" + "\"" + productDiscount.getId() + "\"").executeBlocking().getBody();
    ...
    public void updateById() {
        ProductDiscountFixtures.withUpdateableProductDiscount(productDiscount -> {
            ...
            ProductDiscount updatedProductDiscount = CommercetoolsTestUtils.getProjectRoot().productDiscounts().withId(
                productDiscount.getId()).post(
                    ProductDiscountUpdateBuilder.of().actions(updateActions).version(
                        productDiscount.getVersion()).build()).executeBlocking().getBody();
    ...
    public void updateByKey() {
        ProductDiscountFixtures.withUpdateableProductDiscount(productDiscount -> {
            ...
            String newKey = CommercetoolsTestUtils.randomKey();
            updateActions.add(ProductDiscountSetKeyActionBuilder.of().key(newKey).build());
            ProductDiscount updatedProductDiscount = CommercetoolsTestUtils.getProjectRoot().productDiscounts().withKey(
                productDiscount.getKey()).post(
                    ProductDiscountUpdateBuilder.of().actions(updateActions).version(
                        productDiscount.getVersion()).build()).executeBlocking().getBody();
