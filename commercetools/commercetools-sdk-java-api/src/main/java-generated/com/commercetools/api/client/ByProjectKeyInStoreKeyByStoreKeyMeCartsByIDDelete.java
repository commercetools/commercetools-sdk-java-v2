...
/**
 *  <p>Deletes the Cart for a given <code>id</code> in a Store. Returns a <code>200</code> status if successful.</p>
 ...
 *   <li>If the Cart exists in the Project but does not have either a <code>customerId</code> that matches the <span>customer:{id}</span> scope, or an <code>anonymousId</code> that matches the <span>anonymous_id:{id}</span> scope.</li>
 *  </ul>
 *
 * <hr>
...
public class ByProjectKeyInStoreKeyByStoreKeyMeCartsByIDDelete
        extends TypeApiMethod<ByProjectKeyInStoreKeyByStoreKeyMeCartsByIDDelete, com.commercetools.api.models.cart.Cart>
