...
/**
 *  <p>Creates a Cart in a Store for the Customer or anonymous user. The <code>customerId</code> or <code>anonymousId</code> field on the Cart is automatically set based on the <span>customer:{id}</span> or <span>anonymous_id:{id}</span> scope.</p>
 *  <p>The <code>store</code> field in the created <a href="https://docs.commercetools.com/apis/ctp:api:type:Cart" rel="nofollow">Cart</a> is set to the Store specified by the <code>storeKey</code> path parameter.</p>
