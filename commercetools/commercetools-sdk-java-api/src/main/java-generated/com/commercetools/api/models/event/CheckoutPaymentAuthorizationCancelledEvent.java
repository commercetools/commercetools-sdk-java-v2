
package com.commercetools.api.models.event;

import java.time.*;
import java.util.*;
import java.util.function.Function;

import javax.annotation.Nullable;

import com.commercetools.api.models.subscription.EventSubscriptionResourceTypeId;
import com.fasterxml.jackson.annotation.*;
import com.fasterxml.jackson.databind.annotation.*;

import io.vrap.rmf.base.client.utils.Generated;

import jakarta.validation.Valid;
import jakarta.validation.constraints.NotNull;

/**
 *  <p>Generated when a payment authorization is successfully cancelled in Checkout. This event indicates that the payment is cancelled before it is charged.</p>
 *
 * <hr>
 * Example to create an instance using the builder pattern
 * <div class=code-example>
 * <pre><code class='java'>
 *     CheckoutPaymentAuthorizationCancelledEvent checkoutPaymentAuthorizationCancelledEvent = CheckoutPaymentAuthorizationCancelledEvent.builder()
 *             .id("{id}")
 *             .notificationType("{notificationType}")
 *             .resourceType(EventSubscriptionResourceTypeId.CHECKOUT)
 *             .createdAt(ZonedDateTime.parse("2022-01-01T12:00:00.301Z"))
 *             .data(dataBuilder -> dataBuilder)
 *             .build()
 * </code></pre>
 * </div>
 */
@io.vrap.rmf.base.client.utils.json.SubType("CheckoutPaymentAuthorizationCancelled")
@Generated(value = "io.vrap.rmf.codegen.rendering.CoreCodeGenerator", comments = "https://github.com/commercetools/rmf-codegen")
@JsonDeserialize(as = CheckoutPaymentAuthorizationCancelledEventImpl.class)
public interface CheckoutPaymentAuthorizationCancelledEvent extends Event {

    /**
     * discriminator value for CheckoutPaymentAuthorizationCancelledEvent
     */
    String CHECKOUT_PAYMENT_AUTHORIZATION_CANCELLED = "CheckoutPaymentAuthorizationCancelled";

    /**
     *
     * @return resourceType
     */
    @NotNull
    @JsonProperty("resourceType")
    public EventSubscriptionResourceTypeId getResourceType();

    /**
     *  <p>An object containing details of the payment authorization that was cancelled.</p>
     * @return data
     */
    @NotNull
    @Valid
    @JsonProperty("data")
    public CheckoutMessagePaymentsPayloadBaseData getData();

    /**
     * set resourceType
     * @param resourceType value to be set
     */

    public void setResourceType(final EventSubscriptionResourceTypeId resourceType);

    /**
     *  <p>An object containing details of the payment authorization that was cancelled.</p>
     * @param data value to be set
     */

    public void setData(final CheckoutMessagePaymentsPayloadBaseData data);

    /**
     * factory method
     * @return instance of CheckoutPaymentAuthorizationCancelledEvent
     */
    public static CheckoutPaymentAuthorizationCancelledEvent of() {
        return new CheckoutPaymentAuthorizationCancelledEventImpl();
    }

    /**
     * factory method to create a shallow copy CheckoutPaymentAuthorizationCancelledEvent
     * @param template instance to be copied
     * @return copy instance
     */
    public static CheckoutPaymentAuthorizationCancelledEvent of(
            final CheckoutPaymentAuthorizationCancelledEvent template) {
        CheckoutPaymentAuthorizationCancelledEventImpl instance = new CheckoutPaymentAuthorizationCancelledEventImpl();
        instance.setId(template.getId());
        instance.setNotificationType(template.getNotificationType());
        instance.setResourceType(template.getResourceType());
        instance.setCreatedAt(template.getCreatedAt());
        instance.setData(template.getData());
        return instance;
    }

    public CheckoutPaymentAuthorizationCancelledEvent copyDeep();

    /**
     * factory method to create a deep copy of CheckoutPaymentAuthorizationCancelledEvent
     * @param template instance to be copied
     * @return copy instance
     */
    @Nullable
    public static CheckoutPaymentAuthorizationCancelledEvent deepCopy(
            @Nullable final CheckoutPaymentAuthorizationCancelledEvent template) {
        if (template == null) {
            return null;
        }
        CheckoutPaymentAuthorizationCancelledEventImpl instance = new CheckoutPaymentAuthorizationCancelledEventImpl();
        instance.setId(template.getId());
        instance.setNotificationType(template.getNotificationType());
        instance.setResourceType(template.getResourceType());
        instance.setCreatedAt(template.getCreatedAt());
        instance.setData(
            com.commercetools.api.models.event.CheckoutMessagePaymentsPayloadBaseData.deepCopy(template.getData()));
        return instance;
    }

    /**
     * builder factory method for CheckoutPaymentAuthorizationCancelledEvent
     * @return builder
     */
    public static CheckoutPaymentAuthorizationCancelledEventBuilder builder() {
        return CheckoutPaymentAuthorizationCancelledEventBuilder.of();
    }

    /**
     * create builder for CheckoutPaymentAuthorizationCancelledEvent instance
     * @param template instance with prefilled values for the builder
     * @return builder
     */
    public static CheckoutPaymentAuthorizationCancelledEventBuilder builder(
            final CheckoutPaymentAuthorizationCancelledEvent template) {
        return CheckoutPaymentAuthorizationCancelledEventBuilder.of(template);
    }

    /**
     * accessor map function
     * @param <T> mapped type
     * @param helper function to map the object
     * @return mapped value
     */
    default <T> T withCheckoutPaymentAuthorizationCancelledEvent(
            Function<CheckoutPaymentAuthorizationCancelledEvent, T> helper) {
        return helper.apply(this);
    }

    /**
     * gives a TypeReference for usage with Jackson DataBind
     * @return TypeReference
     */
    public static com.fasterxml.jackson.core.type.TypeReference<CheckoutPaymentAuthorizationCancelledEvent> typeReference() {
        return new com.fasterxml.jackson.core.type.TypeReference<CheckoutPaymentAuthorizationCancelledEvent>() {
            @Override
            public String toString() {
                return "TypeReference<CheckoutPaymentAuthorizationCancelledEvent>";
            }
        };
    }
}
