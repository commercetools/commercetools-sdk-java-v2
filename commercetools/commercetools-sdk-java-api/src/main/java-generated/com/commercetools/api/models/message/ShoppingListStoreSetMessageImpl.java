...
public final class ShoppingListStoreSetMessageImpl implements ShoppingListStoreSetMessage {
    ...
    @JsonCreator
    ShoppingListStoreSetMessageImpl(@JsonProperty("id") final String id, @JsonProperty("version") final Long version,
    ...
    public ShoppingListStoreSetMessageImpl() {
        this.type = SHOPPING_LIST_STORE_SET;
    }
    ...
    public boolean equals(Object o) {
        ...
        ShoppingListStoreSetMessageImpl that = (ShoppingListStoreSetMessageImpl) o;
                ...
                that.lastModifiedBy).append(createdBy, that.createdBy).append(sequenceNumber,
                    that.sequenceNumber).append(resource, that.resource).append(resourceVersion,
                        that.resourceVersion).append(type, that.type).append(resourceUserProvidedIdentifiers,
                            that.resourceUserProvidedIdentifiers).append(store, that.store).isEquals();
    }
    ...
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(id).append(version).append(createdAt).append(lastModifiedAt).append(
            lastModifiedBy).append(createdBy).append(sequenceNumber).append(resource).append(resourceVersion).append(
                type).append(resourceUserProvidedIdentifiers).append(store).toHashCode();
    }
