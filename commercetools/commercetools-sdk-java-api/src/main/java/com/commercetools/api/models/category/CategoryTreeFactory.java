...
public class CategoryTreeFactory {
    ...
    private Map<LocalizedStringEntry, Category> getLocalizedStringEntryCategoryMap(final List<Category> allCategoriesAsFlatList) {
        ...
        allCategoriesAsFlatList.forEach(category -> {
            final Set<Locale> localesForTheCategory = category.getSlug().values().keySet().stream().map(Locale::new).collect(
                    Collectors.toSet());
            localesForTheCategory.forEach(locale -> {
                final LocalizedStringEntry stringsEntry = LocalizedStringEntry.of(locale, category.getSlug().values().get(locale));
