
package com.commercetools.api.models.review;

import org.assertj.core.api.Assertions;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.MethodSource;

public class ReviewSetCustomFieldActionTest {

    @ParameterizedTest(name = "#{index} with {0}")
    @MethodSource("objectBuilder")
    public void buildUnchecked(String name, ReviewSetCustomFieldActionBuilder builder) {
        ReviewSetCustomFieldAction reviewSetCustomFieldAction = builder.buildUnchecked();
        Assertions.assertThat(reviewSetCustomFieldAction).isInstanceOf(ReviewSetCustomFieldAction.class);
    }

    public static Object[][] objectBuilder() {
        return new Object[][] { new Object[] { "name", ReviewSetCustomFieldAction.builder().name("name") },
                new Object[] { "value", ReviewSetCustomFieldAction.builder().value("value") } };
    }

    @Test
    public void name() {
        ReviewSetCustomFieldAction value = ReviewSetCustomFieldAction.of();
        value.setName("name");
        Assertions.assertThat(value.getName()).isEqualTo("name");
    }

    @Test
    public void value() {
        ReviewSetCustomFieldAction value = ReviewSetCustomFieldAction.of();
        value.setValue("value");
        Assertions.assertThat(value.getValue()).isEqualTo("value");
    }
}
