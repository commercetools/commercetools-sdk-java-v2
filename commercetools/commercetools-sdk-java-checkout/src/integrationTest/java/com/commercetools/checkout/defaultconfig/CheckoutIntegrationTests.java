...
public class CheckoutIntegrationTests {
    ...
    public static ProjectApiRoot createApiClient() {
        return ApiRootBuilder.ofEnvironmentVariables()
                .buildProjectRoot();
    }
    ...
    // create client, then cart, then transaction
        ...
        public void createAndGetTransactionTest() {
            ...
            var transaction = checkoutApiRoot.with().transactions().post(
            ...
            Assertions.assertNotNull(checkoutApiRoot.with().transactions().withId(transaction.getId()).get().executeBlocking().getBody());
            Assertions.assertEquals(transactionKey, transaction.getKey());
            Assertions.assertNotNull(checkoutApiRoot.with().transactions().withKey(transaction.getKey()).get().executeBlocking().getBody());
        }
}
