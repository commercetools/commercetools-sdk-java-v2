...
public class ByProjectKeyImportSummariesImportSinkKeyByImportSinkKeyGet {
   ...
   public ApiHttpResponse<com.commercetools.importer.models.importsummaries.ImportSummary> executeBlocking(){
      try {
          return execute().get();
      } catch (Exception e) {
   ...
   public CompletableFuture<ApiHttpResponse<com.commercetools.importer.models.importsummaries.ImportSummary>> execute(){
      return apiHttpClient.execute(this.createHttpRequest())
              .thenApply(response -> {
                  if(response.getStatusCode() >= 400){
                      throw new ApiHttpException(response.getStatusCode(), new String(response.getBody()), response.getHeaders());
                  }
                  return Utils.convertResponse(response,com.commercetools.importer.models.importsummaries.ImportSummary.class);
              });
   }
