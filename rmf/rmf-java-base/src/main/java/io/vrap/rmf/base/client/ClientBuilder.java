...
public class ClientBuilder implements Builder<ApiHttpClient> {
    ...
    public ClientBuilder withRetryMiddleware(final int maxRetries, List<Integer> statusCodes,
        ...
        return withRetryMiddleware(new RetryMiddleware(maxRetries, statusCodes, failures));
    }
    ...
    public ClientBuilder withRetryMiddleware(final int maxRetries, final long delay, final long maxDelay,
        ...
        return withRetryMiddleware(
            new RetryMiddleware(maxRetries, delay, maxDelay, RetryMiddleware.handleFailures(failures)
                    .andThen(RetryMiddleware.handleStatusCodes(statusCodes).andThen(fn))));
    }

    public ClientBuilder withRetryMiddleware(final int maxRetries, final long delay, final long maxDelay,
            final Function<RetryPolicyBuilder<ApiHttpResponse<byte[]>>, RetryPolicyBuilder<ApiHttpResponse<byte[]>>> fn) {
        return withRetryMiddleware(new RetryMiddleware(maxRetries, delay, maxDelay, fn));
    }
