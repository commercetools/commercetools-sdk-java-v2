...
public class SerializerOnlyApiHttpClient implements ApiHttpClient {
    ...
    @Override public <O> CompletableFuture<ApiHttpResponse<O>> execute(ApiHttpRequest request, Class<O> outputType) {
        throw new IllegalStateException(NO_EXECUTABLE_API_CLIENT);
    }

    @Override public <O> CompletableFuture<ApiHttpResponse<O>> execute(ClientRequestCommand<O> method) {
        throw new IllegalStateException(NO_EXECUTABLE_API_CLIENT);
    }

    @Override public ResponseSerializer getSerializerService() {
        return serializer;
    }

    @Override public CompletableFuture<ApiHttpResponse<byte[]>> execute(ApiHttpRequest request) {
        throw new IllegalStateException(NO_EXECUTABLE_API_CLIENT);
    }
