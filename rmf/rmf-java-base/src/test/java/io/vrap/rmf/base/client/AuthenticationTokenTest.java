...
public class AuthenticationTokenTest {
    ...
    public void testExpiredNotSet() throws IOException {
        final AuthenticationToken token = JsonUtils.fromJsonString("{ \"access_token\": \"12345\"}", AuthenticationToken.class);
        Assertions.assertThat(token.isExpired()).isEqualTo(false);
    }

    @Test
    public void testAlreadyExpired() throws IOException {
        final AuthenticationToken token = JsonUtils.fromJsonString("{ \"access_token\": \"12345\", \"expires_in\": 299}", AuthenticationToken.class);
        Assertions.assertThat(token.isExpired()).isEqualTo(true);
    }

    @Test
    public void testNotExpired() throws IOException {
        final AuthenticationToken token = JsonUtils.fromJsonString("{ \"access_token\": \"12345\", \"expires_in\": 360}", AuthenticationToken.class);
        Assertions.assertThat(token.isExpired()).isEqualTo(false);
    }
